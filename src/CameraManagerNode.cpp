...
#include "mv-cameras-ros/sensor/CameraManagerNode.h"
...
#include "mv-cameras-ros/exceptions/SystemException.h"
#include "mv-cameras-ros/exceptions/InvalidOperationException.h"
...
/******************************************************************************/
  ...
  CameraManagerNode::CameraManagerNode(const ros::NodeHandle& nh) :
    ...
    _getCamerasService = _nodeHandle.advertiseService("get_cameras",
      &CameraManagerNode::getCameras, this);
...
/******************************************************************************/
  ...
  bool CameraManagerNode::getCameras(mv_cameras_ros::GetCamerasRequest&
      request, mv_cameras_ros::GetCamerasResponse& response) {
    for (auto it = _cameraNodes.begin(); it != _cameraNodes.end(); ++it) {
      if (it->second->getState().read() == dsPresent) {
        response.serials.push_back(it->first);
      }
    }
    return true;
  }
